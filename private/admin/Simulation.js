var parameters = {
	...
	simRate: 2000 // how fast the simulation runs
};
...
// checks if all live nodes have been CH in last 1/P rounds
function check() {
...
function startFrame() {
	...
	for (var i in clusters) {
		// draw and subtract energy from members to CH
		...
		// draw and subtract energy from CH to BS
		timepassed += (parameters.ri / cluster.nodes.length);
...
function startSimulation() {
	...
	// rdata is for results
	rdata = {};
	rdata.entries = [];
	rdata.startTime = Date.now();
	// let timeinterval = getTcf() + (getFrames().length+1)*parameters.ri;
	CR = 1;
	// start round interval with time difference (equation from notes)
	startRound();
...
function stopSimulation() {
	...
	// ends all intervals and timeouts so that it stops simulating
